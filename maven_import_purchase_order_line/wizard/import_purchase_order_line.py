...
HEADER_ALIASES = {
    ...
    'PUR. FOR': 'PUR. FOR',
    'PUR FOR': 'PUR. FOR',
    'PURCHASE FOR': 'PUR. FOR',
    'ANALYTIC': 'PUR. FOR',
    'EXPECTED ARRIVAL': 'EXPECTED ARRIVAL',
    'DATE PLANNED': 'EXPECTED ARRIVAL',
    'SCHEDULED DATE': 'EXPECTED ARRIVAL',
    'LP': 'LP',
    'LIST PRICE': 'LP',
    'DISC.%': 'DISC.%',
    'DISC%': 'DISC.%',
...
class ImportPurchaseOrderLineWizard(models.TransientModel):
    ...
    def action_import(self):
        ...
        if order.state != 'draft':
            raise UserError(_('Purchase order lines can only be imported on draft RFQs.'))
        if not self.file:
        ...
        not_imported = []
        existing_line_keys = {
            self._line_key(line.product_id.id, line.product_qty, line.price_unit, line.discount)
    ...
    def _parse_datetime(self, value):
        ...
        for fmt in (
            ...
            try:
                ...
                if parsed.year == 1900:
                    parsed = parsed.replace(year=fields.Datetime.now().year)
                return parsed.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            except ValueError:
                continue
        return False
        ...
        for row_idx in range(1, sheet.nrows):
            ...
            for col_idx in range(sheet.ncols):
                ...
                if cell.ctype == xlrd.XL_CELL_DATE:
                    ...
                    row_values.append(value.strftime(DEFAULT_SERVER_DATETIME_FORMAT))
                else:
    ...
    def _split_product_value(self, value):
        ...
        if not text:
            return '', ''
        # Formats: "[CODE] Name", "CODE / Name", plain name/code
        bracket_match = re.match(r'^\[([^\]]+)\]\s*(.*)$', text)
        if bracket_match:
            return bracket_match.group(1).strip(), (bracket_match.group(2) or '').strip() or bracket_match.group(1).strip()
        if ' / ' in text:
    ...
    def _find_analytic_distribution(self, pur_for_value):
        """Map Pur. For value to analytic_distribution if analytic accounts exist."""
        name = self._cell_value(pur_for_value)
        if not name:
            return False, True  # no value, ok
        if 'analytic_distribution' not in self.env['purchase.order.line']._fields:
            return False, True
        ...
        if not account:
            return False, False
        return {str(account.id): 100.0}, True
    ...
    def _process_row(self, order, row, existing_line_keys):
        ...
        pur_for = self._cell_value(row.get('PUR. FOR', ''))
        expected_raw = row.get('EXPECTED ARRIVAL', '')
        expected_arrival = self._parse_datetime(expected_raw)
        price_unit = unit_rate or 0.0
        purchase_for_id = False
        if pur_for:
            purchase_for_id = self.env['purchase.for'].search([('name', '=', str(pur_for))], limit=1)
        ...
        if self._cell_value(expected_raw) and not expected_arrival:
            failure_reasons.append('invalid_expected_arrival')

        if pur_for and not purchase_for_id:
            failure_reasons.append('pur_for_not_found')
        ...
        export_row = {
            ...
            'Quantity': quantity,
            'Unit': unit_name,
            'Pur. For': purchase_for_id and purchase_for_id.id or False,
            'Expected Arrival': self._cell_value(expected_raw) or (expected_arrival or ''),
            'LP': lp_value,
            'Disc.%': discount,
        ...
        if expected_arrival:
            line_vals['date_planned'] = expected_arrival
        elif order.date_planned:
            line_vals['date_planned'] = order.date_planned
        else:
    ...
    def _prepare_not_imported_file(self, not_imported):
        ...
        for row_idx, item in enumerate(not_imported, start=1):
            ...
            sheet.write(row_idx, 3, export_row.get('Pur. For', ''), default_style)
            sheet.write(row_idx, 4, export_row.get('Expected Arrival', ''), default_style)
            sheet.write(row_idx, 5, export_row.get('LP', ''), default_style)
            sheet.write(row_idx, 6, export_row.get('Disc.%', ''), default_style)
            sheet.write(row_idx, 7, export_row.get('Unit Rate', ''), default_style)
            sheet.write(row_idx, 8, export_row.get('Amount', ''), default_style)

            tax_status = export_row.get('tax_status') or []
            if not tax_status:
                sheet.write(row_idx, 9, export_row.get('Taxes', ''), default_style)
            else:
                tax_names = []
                all_found = True
                any_found = False
                ...
                if all_found and any_found:
                    sheet.write(row_idx, 9, tax_text, green_style)
                elif not any_found:
                    sheet.write(row_idx, 9, tax_text, red_style)
                else:
                    mixed_parts = []
                    for status in tax_status:
                        if status['found']:
                    ...
                    sheet.write(row_idx, 9, '; '.join(mixed_parts), red_style)
