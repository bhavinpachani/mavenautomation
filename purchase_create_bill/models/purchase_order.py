...
class PurchaseOrder(models.Model):
    ...
    def action_create_vendor_bill(self):
        ...
        if self.state not in ['purchase', 'done']:
            raise UserError(_('You can only create bills for confirmed purchase orders.'))
        ...
        if not ready_lines and not mixed_scenario:
            raise UserError(_('There are no products to bill. Please receive products first or check your purchase order lines.'))
        ...
        return self._create_vendor_bill_internal(ready_only=False)

    def _create_vendor_bill_internal(self, ready_only=False):
    ...
    def _create_vendor_bill_internal(self, ready_only=False):
        ...
        if not invoice_lines:
            raise UserError(_('There are no products to bill. Please receive products first.'))
